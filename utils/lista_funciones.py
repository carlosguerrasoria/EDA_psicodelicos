...
def tricategorical_analysis(df, directora, cols_categ, relativa = False, muestra_valores = False):
    ...
    for valor in df[col_directora].unique():
        diccionario_multivariante[valor] = df.loc[df[col_directora] == valor, [col3, col2, col1]]
    
    for valor, df in diccionario_multivariante.items():
        print(f"Respuesta {valor}:")
        plot_categorical_relationship_fin(df, col3, col2, col1, relative_freq=relativa, show_values=muestra_valores)
...
def plot_categorical_relationship_fin(df, cat_col1, cat_col2, relative_freq=False, show_values=False, size_group = 5):
    ...
    if len(unique_categories) > size_group:
        ...
        for i in range(num_plots):
            ...
            plt.xlabel(cat_col1)
            plt.ylabel('%' if relative_freq else 'Conteo')
...
def cardinalidad(df_in, umbral_categorica, umbral_continua):
    ...
    resultados = pd.DataFrame(columns=['Cardinalidad', '%_card', 'Clasificacion'])
